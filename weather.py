...
def fetch_xml_feed() -> tuple[int, BeautifulSoup]:
    ...
    logger.info(
        f"Found {len(storm_titles)} active storms"
        if storm_titles
        else "No active storms found"
    )
    if no_storms:
...
def find_cyclones_in_feed(soup) -> list[dict[str, str]]:
    ...
    for title in storm_titles:
        ...
        img_tag = cdata_soup.find(
            "img",
            src=lambda src: "5day_cone_with_line_and_wind" in src if src else False,
        )
        if img_tag:
    ...
    try:
        with (
            ...
            if new_img.size != existing_img.size:
                logger.info(
                    f"Images have different sizes: {new_img.size} vs {existing_img.size}"
                )
                return True
...
def fetch_all_weather_images(
    soup, image_dir: str, threshold: float = 0.001
) -> list[WeatherImage]:
        ...
        if cyclone["speg_model"]:
            ...
            try:
                models_image = process_single_image(
                    models_url, f"{storm_name}_hurricane_models", image_dir, threshold
                )
                ...
                logger.warning(
                    f"Failed to fetch hurricane models for {storm_name}: {e}"
                )
    ...
    if args.threshold is not None:
        ...
        if threshold_env is not None:
            try:
                ...
                parser.error(
                    f"Invalid THRESHOLD value '{threshold_env}'. Must be a valid float."
                )
        else:
    ...
    if no_storms > 0:
        ...
        if new_images:
            ...
            if slack_token and upload_channel:
                ...
                logger.info(
                    "Skipping Slack upload: missing slack_token or upload_channel."
                )
        ...
        static_image = process_single_image(
            static_url, "two_atl_7d0", image_file_path, args.threshold
        ...
        if static_image.is_new:
            ...
            if slack_token and upload_channel:
                ...
                logger.info(
                    "Skipping Slack upload: missing Slack token or upload channel"
                )
            if discord_webhook_url:
