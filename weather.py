    ...
    for title in titles:
        ...
        if img_tag:
            ...
            if match:
                ...
                if storm_type == 'Hurricane' or storm_type == 'Tropical Storm':
                    cyclones.append({'storm_name': storm_name, 'image_url': img_tag['src']})
                    print(f"Found cyclone: {storm_name} with image URL: {img_tag['src'] if img_tag else 'N/A'}")
    return cyclones
...
def generate_cyclone_images(soup, map_name, image_file_path):
    ...
    for cyclone in cyclones:
        ...
        if response.from_cache:
            print(f"{cyclone['storm_name']} image from cache")
        ...
        if not response.from_cache:
            ...
            if not os.path.exists(gif_file_name):
                ...
                print(f"Updated GIF for {all_images} with new frame.")
...
def upload_to_slack_and_discord(images, image_file_name, gif_file_name, slack_token, discord_webhook_url, image_response):
    ...
    etag = image_response.headers.get('ETag', 'No ETag provided')
...
if __name__ == "__main__":
    ...
    if no_storms is False:
        ...
        print(f"Found {len(images)} cyclones in the feed.")
